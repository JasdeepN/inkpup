...
- `R2_ACCOUNT_ID`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, and either `R2_SECRET_ACCESS_KEY` **or** `R2_API_TOKEN` – R2 credentials with permission to manage the public bucket used for media. Generate traditional access keys with the **Workers R2 Storage Write** (or Admin Read & Write) permission group, or create an API token and store its value as `R2_API_TOKEN`. When only the token is present the CI pipeline and server code automatically derive the SHA-256 hash that Cloudflare’s S3 API expects (equivalent to running `printf 'TOKEN_VALUE' | sha256sum`). The workflow now also exchanges the token for short-lived temporary credentials and exports the session component as `R2_SESSION_TOKEN` (mirrored to `AWS_SESSION_TOKEN`) so any AWS SDK client in the job can authenticate. Cloudflare documents the token flow in [Authentication → Get S3 API credentials from an API token](https://developers.cloudflare.com/r2/api/s3/tokens/#get-s3-api-credentials-from-an-api-token) and [Authenticate against R2 API using auth tokens](https://developers.cloudflare.com/r2/examples/authenticate-r2-auth-tokens/).
- Optional: `R2_PUBLIC_HOSTNAME` – defaults to `https://r2.inkpup.ca` so every environment serves the shared bucket via the custom domain. Override only if you intentionally test against the raw R2 endpoint and revert afterwards.
